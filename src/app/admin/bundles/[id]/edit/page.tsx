...
export default function EditBundlePage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/bundles/${id}`)
      ...
      .then(data => {
          setBundle(data);
          // Map items and add price from product details if available
          // We need to fetch product details to get cost price if not stored in bundle item (which it isn't usually)
          // For now, we'll rely on the products list being fetched to populate cost price
          const mappedItems = (data.items || []).map((item: any) => ({
              product_id: item.product_id,
              variant_id: item.variant_id,
              quantity: item.quantity,
              price: parseFloat(item.product_price || 0),
              cost_price: 0 // Will be updated when products load
          }));
  ...
  // Update cost prices once products are loaded
  useEffect(() => {
      if (products.length > 0 && items.length > 0) {
          setItems(prevItems => prevItems.map(item => {
              ...
              return {
                  ...item,
                  cost_price: product ? parseFloat(product.cost_price || 0) : item.cost_price
              };
          }));
      }
  }, [products.length]); // Only run when products are loaded

  // Calculate sum of original prices whenever items change
  useEffect(() => {
      const sum = items.reduce((acc, item) => acc + (item.price * item.quantity), 0);
      setBundle((prev: any) => prev ? ({ ...prev, original_price: sum.toString() }) : null);
  }, [items]);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
        ...
        addToast("Error updating bundle", "error");
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-500">
      ...
      <form id="edit-bundle-form" onSubmit={handleUpdate} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
